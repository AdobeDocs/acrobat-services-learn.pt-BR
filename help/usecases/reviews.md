...
A colaboração remota entre equipes se tornou necessária para muitas empresas durante a pandemia da Covid-19, [compartilhamento e revisão de documentos digitais](https://www.adobe.io/apis/documentcloud/dcsdk/review-and-approval.html) apresenta uma série de desafios para equipes e recursos multifuncionais.
...
Antes de iniciar o código, você deve [criar credenciais](https://www.adobe.com/go/dcsdks_credentials) para a Adobe PDF Embed API e a Adobe PDF Services API. A API de incorporação do PDF é gratuita. A API de serviços PDF é gratuita para uso por seis meses, depois você pode mudar para uma [plano pré-pago](https://www.adobe.io/apis/documentcloud/dcsdk/pdf-pricing.html) a apenas \$0,05 por transação de documento.
...
Configure o projeto Node.js e Express para fornecer arquivos estáticos de uma pasta chamada “public”. Você pode configurar seus modos de projeto, dependendo de suas preferências. Para começar a usar o produto rapidamente, use o [Gerador de aplicativos Express](https://expressjs.com/en/starter/generator.html). Ou se você quiser simplificar as coisas, você pode [começar do zero](https://expressjs.com/en/starter/hello-world.html) e mantenha o código em um único arquivo JavaScript. No projeto de amostra vinculado acima, você está usando a abordagem de um arquivo e mantendo todos os códigos em index.js.

Copie o `pdftools-api-credentials.json` e `private.key` da amostra de código personalizado para o diretório raiz do projeto. Além disso, adicione-os ao arquivo .gitignore, se você tiver um, para que seus arquivos de credencial não sejam enviados acidentalmente a um repositório.

Em seguida, execute `npm install @adobe/documentservices-pdftools-node-sdk` para instalar o SDK do Node.js para Serviços PDF. Importe este módulo e crie o objeto de credenciais de API dentro do seu código (index.js em seu projeto de amostra), após o restante das importações de dependência desta forma:
...
Agora você está pronto para trabalhar [!DNL Acrobat Services] APIs.
...
Primeiro, crie uma pasta dentro de uma pasta de envios e nomeie-a de “rascunhos”. Você armazena os arquivos carregados e os arquivos de PDF convertidos aqui. Em seguida, execute `npm install express-fileupload` para instalar o módulo Express-FileUpload e adicionar o middleware ao Express em seu código:
...
Para fazer upload de arquivos do aplicativo da Web, crie um `index.html` página da web dentro da pasta uploads. Na página, adicione um formulário de upload de arquivo que envia o arquivo ao endpoint /upload:
...
![Captura de tela do recurso de upload de arquivos na página da Web](assets/reviews_1.png)
...
Adicionar um `/download/:file` rota que fornece acesso ao arquivo de PDF carregado para incorporação na página da Web.
...
![Captura de tela da seleção de um arquivo para revisão](assets/reviews_2.png)
...
Depois que um usuário comenta em um documento, ele clica em **Economize.** Por padrão, ao clicar em **Salvar** baixa o arquivo PDF atualizado. Altere esta ação para atualizar o arquivo de PDF atual no servidor.

Adicionar um `/save` ponto final para o código do servidor que substitui o arquivo PDF na pasta uploads/drafts:
...
Comentários e anotações nos documentos de rascunho agora são salvos no servidor. Você pode [leia mais sobre como os retornos de chamada](https://www.adobe.com/devnet-docs/dcsdk_io/viewSDK/howtos_ui.html#callbacks-workflows) ajuste-se ao seu fluxo de trabalho. Por exemplo, [retornos de chamada de status](https://www.adobe.com/devnet-docs/dcsdk_io/viewSDK/howtos_ui.html#status-callback) ajude a lidar com conflitos de arquivo se várias pessoas quiserem revisar e comentar no mesmo documento simultaneamente.
...
Adicione um ponto final chamado /finalize que chama a função para combinar todos os arquivos de PDF dentro do `uploads/drafts` pasta em um `Final.pdf` e, em seguida, faz o download.
...
Você pode usar essas técnicas para permitir a colaboração em seu aplicativo ou explorar [Amostras do SDK do Nó de Serviços PDF](https://github.com/adobe/pdftools-node-sdk-samples) e [Amostras de API incorporadas do PDF](https://github.com/adobe/pdf-embed-api-samples) no GitHub para obter inspiração sobre como usar APIs Adobe.

Pronto para habilitar o compartilhamento e a revisão de documentos em seu próprio aplicativo? Inscreva-se no seu [[!DNL Adobe Acrobat Services]](https://www.adobe.io/apis/documentcloud/dcsdk/gettingstarted.html) conta de desenvolvedor. Acesse o Adobe PDF Embed gratuitamente e aproveite uma avaliação gratuita de seis meses das outras APIs. Após a avaliação, você pode [pré-pago](https://www.adobe.io/apis/documentcloud/dcsdk/pdf-pricing.html) por apenas \$0,05 por transação de documento à medida que sua empresa cresce.
